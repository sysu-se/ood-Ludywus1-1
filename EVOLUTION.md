...
本文档回答 HW2 作业要求的 7 个反思问题。涉及代码引用均为本仓库 `ood-Ludywus1-1/` 下相对路径。
...
测试:`tests/hw2/06-hint-candidates.test.js`、`tests/hw2/07-hint-next.test.js`。

---

## 2. 你认为提示功能更属于 Sudoku 还是 Game?为什么?

**结论:数据在 Sudoku,动作在 Game,两者协作。**
...
所以选择是清楚的:`Sudoku.getCandidates` / `findNextDeducible` 是数据,`Game.applyHint` 是动作。`Game.getCandidates` / `getNextHint` 仅作为 facade 转发,不重复实现。
...
测试:`tests/hw2/08-explore-basic.test.js`(5 个用例覆盖 enter/commit/abandon/reset/独立栈)、`tests/hw2/09-explore-failed-memory.test.js`、`tests/hw2/10-explore-serialization.test.js`。
...
- `tests/hw1/`:HW1.1 五个套件全部 100/100 通过(15 tests),证明 HW2 改动没破坏旧契约。
- `tests/hw2/`:HW2 五个套件 23 tests 全部通过。
  - `06-hint-candidates`:候选数算法正确性(5 用例)
  - `07-hint-next`:naked-single 推定 + Game.applyHint 行为(5 用例)
  - `08-explore-basic`:enter/commit/abandon/reset 与独立栈(5 用例)
  - `09-explore-failed-memory`:失败指纹累积与跨 explore 持续(4 用例)
  - `10-explore-serialization`:复合 transition + failedFingerprints + 探索中状态序列化往返,以及旧 JSON 向后兼容(4 用例)

合计 38 tests,可通过 `cd ood-Ludywus1-1 && npm test` 一次跑全。
